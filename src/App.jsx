...
const skills = [
  ['Java', javaLogo], ['PHP', phpLogo], ['JavaScript', javascriptLogo], ['MySQL', mysqlLogo],
  ['Python', pythonLogo], ['HTML', htmlLogo], ['Angular', angularLogo], ['Linux', linuxLogo],
];
...
const projects = [{
  ...
  liveUrl: 'https://everycentcount.vercel.app/home',
  sourceUrl: 'https://github.com/Eugene838/Every-cent-count',
}];

function SocialIcon({ children }) {
  return <svg viewBox="0 0 512 512" aria-hidden="true">{children}</svg>;
}
  ...
  return <>
    <header className="toolbar">
      ...
      <div className="social-links">
        <a aria-label="Eugene Sim on LinkedIn" href="https://www.linkedin.com/in/eugene-sim-63a63724b/" target="_blank" rel="noreferrer"><SocialIcon><path d="M416 32H32C14 32 0 46 0 64v384c0 18 14 32 32 32h384c18 0 32-14 32-32V64c0-18-14-32-32-32zM135 416H69V202h66v214zM102 173a39 39 0 1 1 0-77 39 39 0 0 1 0 77zm282 243h-66V312c0-25 0-57-35-57-35 0-40 27-40 55v106h-66V202h64v29h1c9-17 31-34 63-34 67 0 79 44 79 102v117z" /></SocialIcon></a>
    ...
    <main>
      ...
      <section className="section projects-section" id="projects" aria-labelledby="projects-title"><p className="section-label" id="projects-title">Projects</p><div className="projects-grid">{projects.map((project) => <article className="project-card" key={project.title}><div className="project-topline"><p>{project.type}</p><span aria-hidden="true">↗</span></div><h2>{project.title}</h2><p className="project-description">{project.description}</p><div className="project-tech" aria-label="Technologies used">{project.tech.map((tech) => <span key={tech}>{tech}</span>)}</div><div className="project-links"><a href={project.liveUrl} target="_blank" rel="noreferrer">View live app <span aria-hidden="true">↗</span></a><a href={project.sourceUrl} target="_blank" rel="noreferrer">Source code</a></div></article>)}</div></section>
