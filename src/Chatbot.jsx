...
const greeting = { role: 'assistant', content: "Hi. I can answer questions about Eugene's skills, experience, projects, education, and contact links." };

function localReply(question) {
  const q = question.toLowerCase();
  if (q.includes('project') || q.includes('finance') || q.includes('every cent') || q.includes('portfolio')) return 'Projects\n\n• Every Cent Count — Personal finance tracker for logging expenses, managing budgets, and understanding daily spending.\n  Stack: JavaScript, Supabase, and Vercel.\n\nhttps://everycentcount.vercel.app/demo';
  ...
  return "I can help with Eugene's skills, experience, projects, education, GitHub, or LinkedIn. Try asking about one of those.";
}

function MessageContent({ content }) {
  const liveUrl = 'https://everycentcount.vercel.app/demo';
  if (!content.includes(liveUrl)) return content;
  const [before, after] = content.split(liveUrl);
  return <>{before}<a href={liveUrl} target="_blank" rel="noreferrer">View demo ↗</a>{after}</>;
}
...
export default function Chatbot() {
  ...
  return <aside className="chatbot">
    ...
    {isOpen && <section id="resume-chat" className="chat-panel" aria-label="Resume assistant">
      ...
      <div className="chat-messages" aria-live="polite">{messages.map((message, index) => <p className={`message ${message.role}-message`} key={`${message.role}-${index}`}><MessageContent content={message.content} /></p>)}</div>
      <form className="chat-form" onSubmit={sendMessage}><label className="sr-only" htmlFor="chat-input">Ask a question</label><input id="chat-input" name="question" value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Ask about skills, projects, or experience" autoComplete="off" /><button type="submit" disabled={!draft.trim()} aria-label="Send question">Send</button></form>
    </section>}
  </aside>;
}
